            ...
            return (
              <div className='notes-item' key = {note._id}>
                ...
                <div className='notes-item-btns flex align-center justify-between'>
                  <div>
                    <button type = "button" className='notes-item-btn' onClick={() => dispatch(removeNote(note._id))}>
                      <ImCancelCircle />
                    </button>
                    <Link to = {`/edit/${note._id}`} className = "notes-item-btn">
                      <FiEdit />
                    </Link>
                  </div>

                  <Link to = {`/note/${note._id}`} className='read-more-btn fs-14'>Read More</Link>
                </div>
              </div>
            )
          })
