...
const EditNoteForm = () => {
  ...
  let tempNote = notes.filter(note => note._id === id);
  ...
  const onSaveNoteClicked = () => {
    if(!titleError && !contentError){
      ...
      const noteId = formData._id;
      const noteTitle = formData.noteTitle;
      const noteContent = formData.noteContent;
      axios.put(`https://notepad-backend-027c.onrender.com/api/edit-note/${noteId}`, { noteTitle, noteContent });
